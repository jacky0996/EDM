...
const formUrl = ref('');
const formId = ref<number | string>(''); // 儲存來自 getDisplayList 的表單 ID

// --- 問卷配置器狀態 (恢復) ---
const config = reactive({
  title: props.eventData?.title || '活動報名表',
  description:
    props.eventData?.summary || '感謝您參加本活動，請填寫以下報名資訊。',
...
async function handleDeleteUrl() {
  ...
  await ElMessageBox.confirm(
    '確定刪除此 Google 表單綁定？此操作將同步移除雲端紀錄。',
    '警告',
    {
      type: 'warning',
...
<template>
  <div v-loading="loading" class="registration-form-container p-2">
    ...
    <div
      v-if="!isDisplay"
      class="flex flex-col items-center justify-center rounded-2xl border border-gray-100 bg-white py-20 italic text-gray-400 shadow-sm"
    >
    ...
    <div
      v-else-if="loadError"
      class="flex flex-col items-center justify-center rounded-2xl border border-gray-100 bg-white py-20 shadow-sm"
    >
      ...
      <p class="mb-4 font-bold text-gray-600">資料加載發生逾時</p>
    ...
    <div v-else class="space-y-6">
          ...
          >
            <div class="flex items-center gap-4">
              ...
              <div class="flex flex-col">
                ...
                <span class="mt-0.5 text-xs font-medium text-gray-400"
                  >系統已自動建立並連結此活動之報名清單</span
                >
              </div>
            </div>

            <div
              class="flex min-w-[280px] flex-1 items-center gap-2 rounded-xl border border-gray-100 bg-gray-50 p-2 shadow-inner transition-all group-hover:border-emerald-200"
            >
              <code
                class="flex-1 truncate px-2 font-mono text-[11px] font-bold text-emerald-700"
                >{{ formUrl }}</code
              >
              <ElButton
                type="primary"
                ...
                plain
                @click="handleCopy"
                class="border-gray-200 !bg-white hover:!border-emerald-500"
              >
                <template #icon>
            ...
            <div class="flex items-center gap-3">
              <a :href="formUrl" target="_blank" class="no-underline">
                <ElButton
                  type="primary"
                  class="!h-10 !rounded-xl px-6 font-bold shadow-md shadow-emerald-500/10 transition-all hover:shadow-emerald-500/20 active:scale-95"
                >
                  開啟表單 ↗
                </ElButton>
              </a>
              ...
              <ElButton
                type="success"
                class="!h-10 !rounded-xl px-5 font-bold shadow-sm"
      ...
      <!-- B. 配置器區塊 (未產製/編輯模式時顯示) -->
      <div v-if="!isCreatedOrBound" class="animate-fade-in space-y-6">
        <div class="flex items-center justify-between px-2">
          <div class="flex flex-col">
            <h3 class="text-xl font-bold text-gray-800">產製 Google 表單</h3>
            <p class="text-sm text-gray-500">
              設定完畢後點擊下方按鈕，系統將自動在雲端建立表單
            </p>
          </div>
          <div class="flex items-center gap-2">
            <span class="animate-pulse text-sm font-bold text-red-500"
              >* 請先產製或綁定問卷</span
            >
          </div>
        </div>
        ...
        <div class="grid grid-cols-1 gap-6 lg:grid-cols-12">
          ...
          <div class="space-y-6 lg:col-span-4">
            <ElCard shadow="never" class="!rounded-2xl border-gray-200">
              <template #header>
                <span class="font-bold">1. 表單資訊</span>
              </template>
              <ElForm label-position="top">
                <ElFormItem label="表單主標題">
                  <ElInput v-model="config.title" />
                </ElFormItem>
                <ElFormItem label="介紹說明">
                  <ElInput
            ...
            <ElCard shadow="never" class="!rounded-2xl border-gray-200">
              <template #header>
                <span class="font-bold">2. 基本欄位勾選</span>
              </template>
              <ElCheckboxGroup
                v-model="config.standardFields"
          ...
          <div class="lg:col-span-8">
            ...
            >
              ...
              <div class="space-y-4">
                <div
                  ...
                  :key="index"
                  class="relative rounded-xl border-2 border-gray-100 bg-white p-6 transition-colors hover:border-blue-300"
                >
                  <div class="flex items-start gap-4">
                    ...
                    <ElButton
                      type="danger"
                      link
                      @click="config.customQuestions.splice(index, 1)"
                    >
                      移除
                    </ElButton>
                  </div>
                  ...
                  <div
                    ...
                    class="mt-4 space-y-2 border-l-2 border-blue-200 pl-4"
                  >
                    <div
            ...
            <div class="mt-8">
              <ElButton
                type="primary"
                class="!h-16 w-full !rounded-2xl text-xl font-bold shadow-xl"
                :loading="loading"
