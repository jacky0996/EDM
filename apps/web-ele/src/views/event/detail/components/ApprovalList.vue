...
async function init() {
  ...
  filteredList.value = [];
  try {
    const displayRes: any = await requestClient.post(
    ...
    list.value = responses.map((item: any) => mapResponse(item));
    applyFilter();
  } catch (error) {
    console.error('ApprovalList init failed:', error);
...
function mapResponse(item: any) {
  ...
  if (Array.isArray(item.answers)) {
    item.answers.forEach((ans: any) => {
      ...
      else if (title.includes('電話') || title.includes('手機'))
        row.mobile = answer;
      else if (title.includes('公司')) row.company = answer;
      else if (title.includes('郵件') || title.toLowerCase().includes('email'))
        row.email = answer;
    });
  }
  return row;
...
async function updateStatus(row: any, status: number) {
  try {
    ...
    const res: any = await requestClient.post(
      '/edm/event/updateResponseStatus',
      {
    ...
    ElMessage.error(error.message || '連線異常');
  } finally {
    loading.value = false;
  }
}
...
<template>
  <div class="approval-list rounded-2xl bg-white">
    <!-- 尚未建立 Google 表單 -->
    <div v-if="noBoundForm" class="py-20">
      <ElEmpty
        description="尚未建立 Google 報名表，請先至「報名表」頁籤進行產製"
      />
    </div>
      ...
      <div class="mb-6 flex items-center justify-between">
        ...
        <div class="flex items-center gap-4 text-sm text-gray-500">
          <template v-if="isApproveMode">
            <span
              >待審核：{{
                filteredList.filter((r) => r.status === 0).length
              }}
              筆</span
            >
            <span class="text-green-600"
              >已通過：{{
                filteredList.filter((r) => r.status === 1).length
              }}
              筆</span
            >
            <span class="text-red-500"
              >不通過：{{
                filteredList.filter((r) => r.status === 2).length
              }}
              筆</span
            >
          </template>
        ...
        <ElTableColumn label="提交時間" width="165">
          <template #default="{ row }">
            <span class="text-xs text-gray-500">
              {{
                row._createTime
                  ? String(row._createTime).replace('T', ' ').substring(0, 19)
                  : '-'
              }}
            </span>
          </template>
          ...
          <template #default="{ row }">
            <div class="flex flex-wrap items-center justify-center gap-2">
              ...
              <template v-if="isApproveMode && row.status === 0">
                ...
                <ElButton type="danger" size="small" @click="handleReject(row)">
                  拒絕
                </ElButton>
              </template>
              <span
                v-else-if="isApproveMode"
    ...
    >
      <div v-if="currentResponse" class="flex flex-col">
        <!-- 核心資訊摘要 -->
        <div
          class="mb-6 grid grid-cols-2 gap-3 rounded-xl border border-gray-100 bg-gray-50 p-4"
        >
          ...
          <div class="flex flex-col">
            <span
              ...
              >電子郵件</span
            >
            <span class="break-all font-medium text-gray-700">{{
              currentResponse.email || '---'
            }}</span>
          </div>
          <div class="flex flex-col">
            <span
        ...
        <div class="px-1">
          <h4
            class="mb-3 flex items-center gap-2 text-sm font-bold text-gray-500"
          >
            <div class="h-4 w-1 rounded-full bg-emerald-500"></div>
            完整填寫內容
          </h4>
          <div
            class="custom-scrollbar max-h-[400px] overflow-y-auto rounded-xl border border-gray-100 pr-2"
          >
            <ElDescriptions :column="1" border>
              <ElDescriptionsItem
                v-for="(ans, i) in currentResponse._rawAnswers"
                :key="i"
                :label="ans.title"
              >
                <span class="whitespace-pre-wrap font-medium text-gray-700">{{
                  ans.answer || '(未填寫)'
                }}</span>
              </ElDescriptionsItem>
            </ElDescriptions>
        ...
        <!-- 底部元數據 -->
        <div
          class="mt-6 flex justify-between border-t border-gray-50 px-2 pt-4 text-[10px] text-gray-300"
        >
